...
const totalPages = 2
  ...
  const currentResults = resultsRows.slice(
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div className="relative flex-1 w-full">
              <SelectDropdown
                value={exam}
                onChange={setExam}
                options={[{ label: "UI Designer Level 1 Assessment", value: "UI Designer Level 1 Assessment" }, ...examOptions]}
              />
            </div>
  ...
  {kpis.map((kpi, i) => (
    <div
      key={i}
      className="bg-card rounded-xl pt-1 px-3 pb-3 border border-slate-600"
    >
      <div className="flex items-center justify-between ">
        <span className="text-slate-300 text-md font-bold leading-tight">
          {kpi.title}
        </span>
      ...
      <div className="text-2xl font-bold text-white mt-1 mb-1">
        {kpi.value}
      </div>
    ...
    <table className="w-full">
      ...
      <tbody className="bg-slate-800">
        {currentResults.map((student) => {
          const gradeColor =
            gradeLegend.find((g) => g.code === student.grade)?.color || "#94a3b8";
          return (
            <tr
              key={student.id}
...
>
  {student.violations}
              ...
              <span
  className={`inline-flex items-center justify-center min-w-[2rem] h-6 px-2 rounded-full text-xs font-medium text-white ${
    student.grade === "A1" ? "bg-[#7dafc6]/30" :
    student.grade === "A2" ? "bg-[#ff1818]/30" :
    student.grade === "B1" ? "bg-[#4bd164]/30" :
    student.grade === "B2" ? "bg-[#fbbf24]/30" :
    student.grade === "C1" ? "bg-[#ef4444]/30" :
    "bg-slate-500/30"
  }`}
>
  {student.grade}
             ...
             <td className="px-6 py-4">
  <button className="p-2 rounded-lg border border-white hover:bg-white/10 transition-colors flex items-center justify-center">
